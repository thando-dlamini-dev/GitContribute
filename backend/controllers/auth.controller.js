...
import { fetchUserProfile, getUserRepoData, searchReposByTechStack } from "../lib/octokit.config.js";
...
export const generateReposWithAi = async (req, res) => {
  try {
   ...
   const lowercaseTechStack = techStackData.techStack.map(tech => tech.toLowerCase());

   const techStack  = lowercaseTechStack.map(tech => tech.toLowerCase()).filter(tech => !excludeTerms.includes(tech));  // Convert to lowercase.filter(tech => !excludeTerms.includes(tech)); // Filter out unwanted terms


    // Search for repos
    const repos = (await searchReposByTechStack(techStack, userToken)).map(repo => repo);
